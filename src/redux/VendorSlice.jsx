  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getVendors.fulfilled, (state, action) => {
        state.isLoading = false;
        // Combine all vendors from both categories
        const allVendors = [
          ...(action.payload.Restaurant || []),
          ...(action.payload.Supermarket || []),
        ];
        state.vendors = allVendors;
        state.count = allVendors.length;
        // Remove pagination since this API doesn't use it
        state.pageIndex = 1;
        state.pageSize = allVendors.length;
      })
