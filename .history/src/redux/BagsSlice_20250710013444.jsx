...
export const BagsSlice = createSlice({
  ...
  reducers: {
    ...
    toggleBagFavorite: (state, action) => {
      const { bagId, isCurrentlyFavorited } = action.payload;
      state.bags = state.bags.map((bag) => {
        const bagIdToCheck = bag.id || bag.bagId || bag._id;
        if (bagIdToCheck?.toString() === bagId?.toString()) {
          return {
            ...bag,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllBags.fulfilled, (state, action) => {
        state.loading = false;
        const newBags = action.payload.data || [];
        const existingBags = state.bags;
