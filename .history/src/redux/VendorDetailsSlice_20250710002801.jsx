...
export const getVendorDetails = createAsyncThunk(
  ...
  async (vendorId, thunkAPI) => {
    try {
      ...
      // Try using a direct vendor details endpoint
      const response = await axios.get(
        `${BaseUrl}api/Vendors/${vendorId}`,
        config
      );

      console.log("Vendor details response:", response.data);

      // Return the vendor with empty similarItems since we don't have a specific endpoint
      return {
        ...response.data,
