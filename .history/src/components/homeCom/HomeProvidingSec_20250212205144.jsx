...
function Section({ currentIndex }) {
  return (
    <div className="relative flex flex-col items-center justify-center h-64 text-center transition-opacity duration-500">
      {data.map((item, index) => (
        <div
          key={index}
  ...
  return (
    <section>
      <div className={`relative`}>
        ...
        <button
          onClick={nextSlide}
          className="absolute right-2 top-1/2 -translate-y-1/2 p-3 bg-gray-800 text-white rounded-full shadow-md hover:bg-gray-700 transition">
          ❯
        </button>
      </div>
