...
function Section({ currentIndex, data, prevSlide, nextSlide }) {
  return (
    <div className="flex flex-col relative lgHome:w-12/12 w-9/12 font-sans items-center justify-center text-center transition-opacity duration-500">
      {data.map((item, index) => (
        <div
          key={index}
          className={`bg-darkBiege absolute w-/12 h-96 px-24 py-24 transition-opacity duration-500 rounded-3xl ${
      ...
      <div className="text-5xl flex justify-between w-5/12 absolute lgHome:bottom-10 -bottom-2 stransform -translate-y-1/2 ">
        <WestIcon
          onClick={prevSlide}
          fontSize="inherit"
