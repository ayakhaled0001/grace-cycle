...
function Section({ currentIndex, data }) {
  return (
    <div className="relative flex flex-col font-sans items-center justify-center text-center transition-opacity duration-500">
      {data.map((item, index) => (
        <div
          key={index}
...
function HomeProvidingSec() {
  ...
  return (
    <section>
      <div className={`bg-darkBiege`}>
        <div>
          <button
            onClick={prevSlide}
            className=" -translate-y-1/2 p-3 bg-gray-800 text-black rounded-full shadow-md hover:bg-gray-700 transition">
            prev
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-3 bg-gray-800 text-black rounded-full shadow-md hover:bg-gray-700 transition">
            next
          </button>
        </div>
