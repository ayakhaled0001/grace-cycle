...
function Section({ currentIndex, data, prevSlide, nextSlide }) {
  return (
    <div className="flex flex-col relative w-6/12 font-sans items-center justify-center text-center transition-opacity duration-500">
      ...
      <div className="text-5xl flex justify-between w-6/12 absolute bottom-10 stransform -translate-y-1/2">
        <WestIcon
          onClick={prevSlide}
          fontSize="inherit"
          className="cursor-pointer text-paleWhite hover:text-darkGreen transition-colors"
        />
        <EastIcon
          onClick={nextSlide}
          fontSize="inherit"
          className="cursor-pointer text-paleWhite"
        />
      </div>
    </div>
  );
