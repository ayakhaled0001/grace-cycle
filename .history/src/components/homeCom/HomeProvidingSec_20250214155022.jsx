...
function Section({ currentIndex, data, prevSlide, nextSlide }) {
  return (
    <>
      <div className="flex flex-col relative w-6/12 h-58 font-sans items-center justify-center text-center transition-opacity duration-500 -z-10">
        {data.map((item, index) => (
          <div
            key={index}
            className={`bg-darkBiege absolute w-12/12 h-80 px-16 py-16 transition-opacity duration-500 rounded-3xl inset-0 ${
        ...
        <button onClick={prevSlide} className="absolute left-0 cursor-pointer">
          <WestIcon />
        </button>
        <button onClick={nextSlide} className="absolute right-0 cursor-pointer">
          <EastIcon />
        </button>
      </div>
    </>
