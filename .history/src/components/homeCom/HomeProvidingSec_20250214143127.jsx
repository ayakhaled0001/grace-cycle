...
function Section({ currentIndex, data }) {
  return (
    <div className=" bg-darkBiege flex flex-col font-sans items-center justify-center text-center transition-opacity duration-500 relative">
      {data.map((item, index) => (
        <div
          key={index}
          className={`bg-darkBiege absolute w-full transition-opacity duration-500 ${
...
function HomeProvidingSec() {
  ...
  return (
    <section>
      <div className={` w-5/12 h-14`}>
        <Section currentIndex={currentIndex} data={data} />
      </div>
      <div>
        <button onClick={prevSlide} className=" p-3   transition">
          prev
        </button>
        <button onClick={nextSlide} className=" p-3 transition">
          next
        </button>
      </div>
    </section>
