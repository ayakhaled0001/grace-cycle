    ...
    <>
      <div className="flex flex-col relative w-6/12  font-sans items-center justify-center text-center transition-opacity duration-500 -z-10">
        {data.map((item, index) => (
          <div
            key={index}
            className={`bg-darkBiege absolute w-9/12 h-96 px-24 py-24 -z-10 transition-opacity duration-500 rounded-3xl  ${
        ...
        <div className="text-6xl relative z-40">
          <WestIcon
            onClick={prevSlide}
            fontSize="inherit"
            className="absolute left-0 cursor-pointer text-paleWhite z-30"
          />
          <EastIcon
            onClick={nextSlide}
            fontSize="inherit"
            className="absolute right-0 cursor-pointer text-paleWhite z-30"
          />
        </div>
      </div>
    </>
...
function HomeProvidingSec() {
  ...
  return (
    <section className="relative">
      ...
      <div className="absolute w-3/12  bg-darkGreen  bottom-0 right-0 -z-10 top-48"></div>
    </section>
  );
}
