...
function Section({ currentIndex, data }) {
  return (
    <div className="flex flex-col  w-6/12 h-96 font-sans items-center justify-center text-center transition-opacity duration-500 relative -z-10">
      {data.map((item, index) => (
        <div
          key={index}
          className={`bg-darkBiege absolute w-12/12 h-96 px-16 py-16 transition-opacity duration-500 rounded-3xl ${
...
function HomeProvidingSec() {
  ...
  return (
    <section className="">
      <div className="bg-darkGreen w-80 h-44 rounded-tr-full rounded-br-full relative -z-10"></div>
