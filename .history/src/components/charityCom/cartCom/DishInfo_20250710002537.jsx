...
function DishInfo({ itemId, itemType = "dish", showShoppingCart = true }) {
  ...
  // Get full food listing state for debugging
  const fullFoodListingState = useSelector((state) => state.foodListing);

  // Get vendor data from vendorFilter to check ID mapping
  const { allVendors } = useSelector((state) => state.vendorFilter);
  ...
  if (itemType === "dish") {
    ...
    return (
      <>
        ...
        {console.log(
          "DishInfo - Full food listing state:",
          fullFoodListingState
        )}
        {console.log("DishInfo - All vendor-related properties:", {
          vendorId,
          vName,
          vendorOpeningTime,
          vendorClosingTime,
          // Check if there are any other vendor-related fields
          allKeys: Object.keys(fullFoodListingState),
        })}
        {console.log("DishInfo - VendorFilter allVendors:", allVendors)}
        {console.log("DishInfo - Looking for vendor with vendorId:", vendorId)}
        {console.log(
          "DishInfo - Found vendor in vendorFilter:",
          allVendors?.find(
            (v) =>
              v.userId === vendorId ||
              v.id === vendorId ||
