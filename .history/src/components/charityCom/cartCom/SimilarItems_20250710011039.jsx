...
export default function SimilarItems() {
  ...
  const handleToggleFavorite = (foodId, isCurrentlyFavorited) => {
    // Dispatch the toggle action
    dispatch(toggleFavorite({ foodId, isCurrentlyFavorited }));

    // Update the foodListing state to keep it in sync
    dispatch(
      updateFavoriteStatus({ foodId, isFavourite: !isCurrentlyFavorited })
    );

    // If adding to favorites, add the food data to favorites
    if (!isCurrentlyFavorited) {
      const foodItem = similarItems.find((item) => item.id === foodId);
      if (foodItem) {
        const foodData = {
          id: foodItem.id,
          name: foodItem.name,
                      ...
                      <span className="shadow-xl rounded-full bg-semiDarkBeige p-2 mob470:p-3 absolute -left-3 mob470:-left-4 -top-8 mob470:-top-10">
                        <FavoriteOutlinedIcon
                          className={`cursor-pointer text-lg mob470:text-xl ${
                            favoriteFoods.some((fav) => fav.id === food.id)
                              ? "text-btnsGreen"
                              : "text-paleBarkYellow"
                          }`}
                          onClick={() => {
                            ...
                            handleToggleFavorite(food.id, isCurrentlyFavorited);
                          }}
                        />
                      </span>
