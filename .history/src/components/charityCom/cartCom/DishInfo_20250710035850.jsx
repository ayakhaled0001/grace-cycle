...
function DishInfo({ itemId, itemType = "dish", showShoppingCart = true }) {
  ...
  // Find the item from all categories for bag type (moved up for handlers to access)
  ...
  const handleIncrement = () => {
    if (quantity < (fallbackItem?.quantity || 1)) setQuantity(quantity + 1);
  };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      if (itemType === "bag") {
        // Add bag to cart
        dispatch(addBagToCartState({ bagId: item.id, quantity }));
      } else {
        // Add dish to cart
      ...
      setTimeout(() => {
        window.location.href = "/CharityPage/cart";
      }, 1500);
    } catch (error) {
