...
import { updateBags, toggleBagFavorite } from "../../../redux/BagsSlice";
...
function DishInfo({ itemId, itemType = "dish", showShoppingCart = true }) {
  ...
  const handleToggleFavorite = (itemId, isCurrentlyFavorited) => {
    ...
    if (itemType === "dish" && id) {
      ...
      // For bags, we need to update the bags state directly
      // Since bags don't have their own favorite API, we'll use the food favorite API
      // but also update the bags state to mark it as favorite
      dispatch(toggleFavorite({ foodId: itemId, isCurrentlyFavorited }));

      // Update the bags state to mark this bag as favorite
      // This will make it appear in the FavoriteItems component
      const updatedBags = bags.map((bag) => {
        const bagId = bag.id || bag.bagId || bag._id;
        if (bagId.toString() === itemId.toString()) {
          console.log("Updating bag favorite status:", {
            bagId,
            itemId,
      ...
      console.log(
        "Bags with isFavourite:",
        updatedBags.filter((bag) => bag.isFavourite)
      );
      console.log(
        "Bags with isFav:",
        updatedBags.filter((bag) => bag.isFav)
      );

      // Dispatch an action to update the bags state
      dispatch(updateBags(updatedBags));
    } else {
