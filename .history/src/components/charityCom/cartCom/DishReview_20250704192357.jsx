...
function DishReview({ contains, reviews, reviewsSummary, useVendorReviews }) {
  // If useVendorReviews is true, get reviews from Redux vendorDetails slice
  const vendorReviews = useVendorReviews
    ? useSelector((state) => state.vendorDetails.reviews)
    : undefined;
  const vendorReviewsSummary = useVendorReviews
    ? useSelector((state) => state.vendorDetails.reviewsSummary)
    : undefined;

  const displayReviews = useVendorReviews ? vendorReviews : reviews;
  const displayReviewsSummary = useVendorReviews
    ? vendorReviewsSummary
    : reviewsSummary;

  // Fallback: if nothing provided, use empty array
  const safeReviews = displayReviews || [];
  // Fallback: if nothing provided, use null
  const safeReviewsSummary = displayReviewsSummary || null;
          ...
          <div className="flex flex-col sm:flex-row gap-3 mob470:gap-4 mob560:gap-4 md:gap-6">
            <div className="flex flex-col gap-2 mob470:gap-3 mob560:gap-3 md:gap-4 mb-3 mob470:mb-4 mob560:mb-4 w-full sm:w-2/6">
              ...
              <div className="flex flex-col">
                <div className="flex">
                  <StarRoundedIcon className="text-semiBrightYellow text-base mob470:text-lg mob560:text-lg md:text-xl" />
                  <StarRoundedIcon className="text-semiBrightYellow text-base mob470:text-lg mob560:text-lg md:text-xl" />
                  <StarRoundedIcon className="text-semiBrightYellow text-base mob470:text-lg mob560:text-lg md:text-xl" />
                  <StarRoundedIcon className="text-semiBrightYellow text-base mob470:text-lg mob560:text-lg md:text-xl" />
                  <StarHalfRoundedIcon className="text-semiBrightYellow text-base mob470:text-lg mob560:text-lg md:text-xl" />
                </div>
              </div>
            </div>
