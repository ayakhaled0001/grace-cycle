...
import { toggleVendorFavorite } from "../../../redux/VendorFilterSlice";
import { updateFavoriteStatus } from "../../../redux/VendorDetailsSlice";

const VendorInfoCard = ({ vendor }) => {
  const dispatch = useDispatch();
  const { favoriteVendors } = useSelector((state) => state.vendorFilter);

  if (!vendor) return null;

  // Check if vendor is currently favorited
  const isVendorFavorited = favoriteVendors.some(
    (fav) => fav.userId === vendor.userId || fav.id === vendor.userId
  );

  // Favorite toggle handler
  const handleToggleVendorFavorite = () => {
    const isCurrentlyFavorited = isVendorFavorited;
    dispatch(
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center gap-6 px-6 lg:px-8">
      ...
      <div className="w-full lg:w-5/12 flex justify-center relative">
        {/* Favorite Icon */}
        <span className="shadow-xl rounded-full bg-semiDarkBeige p-2 mob470:p-3 absolute -left-4 -bottom-2 z-10">
          <FavoriteOutlinedIcon
            sx={{ fontSize: { xs: 48, sm: 64 } }}
            className={`cursor-pointer ${
              isVendorFavorited ? "text-btnsGreen" : "text-paleBarkYellow"
            }`}
            onClick={handleToggleVendorFavorite}
          />
        </span>
        <img
          src={vendor.picUrl}
