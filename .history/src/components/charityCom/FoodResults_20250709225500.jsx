...
const FoodResults = ({ onClear }) => {
  ...
  // Helper to get a valid food ID
  const getFoodId = (food) =>
    food.id || food.foodId || food._id || food.ID || food.Id;
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 justify-items-center w-full lg:w-[85%] mx-auto">
            {allFoods.map((food) => {
              ...
              if (!foodId) return null; // skip if no valid id
              // console.log('Food card:', food); // Uncomment for debugging
              return (
                <div
                    ...
                    <div className="flex justify-between py-2">
                      ...
                      <div className="">
                        <span className="text-sm px-1 line-through">
                          EGP
                          {food.unitPrice || food.originalPrice || food.price}
                        </span>
                        <span className="text-btnsGreen font-semibold text-lg">
                          EGP
                          {food.newPrice || food.discountedPrice || food.price}
                        </span>
                      </div>
                    </div>

                    <Link
                      to={`/CharityPage/cart/${foodId}`}
