...
function CartPage() {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      } else if (fetchUserCart.rejected.match(resultAction)) {
        console.error("Failed to fetch cart:", resultAction.payload);
        // Handle specific error cases
        if (
          resultAction.payload?.includes("403") ||
          resultAction.payload?.includes("Forbidden")
        ) {
          console.error(
            "403 Forbidden - User may not have permission to access cart"
          );
